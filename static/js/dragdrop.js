...
//		transEndEventName = transEndEventNames[ Modernizr.prefixed( 'transition' ) ],
	...
	function getOffset( el ) {
		...
		return { top : offset.top + scrollY(), left : offset.left + scrollX(), bottom: offset.top + scrollY() + el.offsetHeight, right: offset.left + scrollX() + el.offsetWidth }
	}
	...
	Droppable.prototype.options = {
		onDrop : function(instance, draggableEl, changed) { return false; },
		onEnter : function(instance, draggableEl) { return false; },
		onLeave : function(instance, draggableEl) { return false; }
	}

	// based on http://stackoverflow.com/a/2752387 : checks if the droppable element is ready to collect the draggable: the draggable element must intersect the droppable in 90% of its width or height.
	Droppable.prototype.isDroppable = function( draggableEl ) {
		...
		return (offset2.left - this.options.dropMargin <= offset1.left && 
				offset2.right + this.options.dropMargin >= offset1.right && 
				offset2.top - this.options.dropMargin <= offset1.top &&
				offset2.bottom + this.options.dropMargin >= offset1.bottom );
	}
	...
	Droppable.prototype.highlight = function( draggableEl ) {
		if( this.isDroppable( draggableEl ) ) {
			...
			if(this.over) {
				this.over = false;
				this.options.onLeave( this, draggableEl );
			}
		}
	}

	// accepts a draggable element...
	Droppable.prototype.collect = function( draggableEl, changed ) {
		this.options.onDrop( this, draggableEl, changed );
	}
	...
	function Draggable( draggableEl, options ) {
		this.el = draggableEl;
		this.options = extend( {}, this.options );
		extend( this.options, options );
		this.droppables = [];
	...
	Draggable.prototype.options = {
		...
		helper : false,
		// callbacks
		onStart : function(instance) { return false; },
		onDrag : function() { return false; },
		onEnd : function(wasDropped) { return false; },
		testChanged: function(droppableEl, draggableEl) { return true; }
	}
	...
	Draggable.prototype.updateDroppables = function(droppables) {
	...
	Draggable.prototype.onDragEnd = function( instance, event, pointer ) {
		...
		for( var i = 0, len = this.droppables.length; i < len; ++i ) {
			...
			if( droppableEl.isDroppable( instance.element ) ) {
				dropped = true;
				changed = this.options.testChanged(droppableEl.el, instance.element );
				droppableEl.collect( instance.element, changed );
			}
		}
